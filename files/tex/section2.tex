...
The attack script was written with Scapy. It waits for a packet from the client to the server, extracts the client source port, reads the TCP sequence number, and calculates the next expected sequence number \cite{rfc9293,heaton2020}. Since TCP sequence numbers count bytes, the script uses:
